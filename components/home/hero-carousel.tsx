        ...
        <div className="container-page w-full pb-20 lg:pb-28">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="max-w-2xl"
            >
              <Badge className="bg-primary/10 border-primary/30" variant={slide.badge === "Breaking News" ? "live" : "outline"}>
                {slide.badge}
              </Badge>
              <h1 className="mt-4 font-display text-4xl font-bold leading-[1.05] sm:text-5xl lg:text-6xl">
                {slide.title}
              </h1>
              <p className="mt-3 text-base font-medium text-accent">  {slide.subtitle}</p>
            ...
            <div className="flex gap-2">
              {heroes?.map((s, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${i === index ? "w-8 bg-primary-light" : "w-3 bg-white/25 hover:bg-white/40"
                    }`}
                />
              ))}
            </div>
